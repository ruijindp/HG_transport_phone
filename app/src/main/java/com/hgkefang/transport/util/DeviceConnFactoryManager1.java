 ...
 */
public class DeviceConnFactoryManager1 {
    ...
    private static DeviceConnFactoryManager1[] deviceConnFactoryManagers = new DeviceConnFactoryManager1[4];
    ...
    public static DeviceConnFactoryManager1[] getDeviceConnFactoryManagers() {
    ...
    public static void closeAllPort() {
        for (DeviceConnFactoryManager1 deviceConnFactoryManager : deviceConnFactoryManagers) {
    ...
    private DeviceConnFactoryManager1(Build build) {
    ...
    public static final class Build {
        ...
        public DeviceConnFactoryManager1 build() {
            return new DeviceConnFactoryManager1(this);
        }
    }
    ...
    private void queryPrinterCommand() {
        ThreadPool1.getInstantiation().addTask(new Runnable() {
            @Override
            public void run() {
                //发送ESC查询打印机状态指令
                sendCommand = esc;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case READ_DATA:
                    ...
                    String status = context.getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                        if (currentPrinterCommand == null) {
                            ...
                            if (result == 0) {//打印机状态查询
                                ...
                                if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                    status += " " + context.getString(R.string.str_printer_out_of_paper);
                                }
                                if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                    status += " " + context.getString(R.string.str_printer_open_cover);
                                }
                                if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                    status += " " + context.getString(R.string.str_printer_error);
                                }
